...
\begin{document}
    ...
    There are two mayor forms to express the syntax of natural language: \deflab{dependency graphs} and \deflab{constituent trees}.
...
%    We will sometimes consider constituent trees where each constituent is extended by a head assignment, that also includes a head-dependet relation.
...
%    For each word in the sentence and for each constituent where the word is the head, the head of each child constituent is then a dependent if it is inequal.
    ...
    We call such a consituent tree \deflab{discontinuous}.
    In the first one, all consituents are contiguous and we call it \deflab{continuous}.\footnote{
    ...
    The example shows a well known phenomenon in the syntax of English language, called \cn{wh}-movement that occurs in phrases with interrogative words.
    ...
    There are \deflab{treebanks}, large collections of constituent trees that were created manually or semi-automatic, that give an impression in what frequency such phenomena occur in these languages. \citep{Marcus94,EvaKal11,Skut98,Brants04,Noo13}
    ...
    Their aim was to explain the syntax of natural languages using \deflab{context-free grammars}, the second out of four levels of grammars in the hierarchy.
    ...
    The grammar formalism for the next higher level in the hirarchy, called \deflab{context-sensitive grammars}, was dismissed for the modelling and analysis of natural languages due to its complexity.
    ...
    They were later brought under the umbrella term \deflab{mildly context-sensive grammars}, a class that supersedes context-free and is well included in context-sensitive grammars.
    ...
    It assumes an underlying \deflab{lexical} grammar formalism, i.e.\@ one that does contain exactly one lexical symbol in each of its rules.
    ...
    Right after prediction, each blueprint's wildcard symbol is replaced with the sentence position it was predicted for; the obtained object is called a \deflab{supertag}.
    ...
    \Cref{chp:extraction} defines two formalisms for supertags and extraction procedures:
    \begin{compactitem}
    ...
    \Cref{chp:experiments} unrolls the experiments that we conducted to assess the presented extraction and parsing procedures.
