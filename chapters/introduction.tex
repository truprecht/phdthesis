...
\begin{document}
    ...
    The analysis and illustration of a natural language phrase in such an expression is called \deflab{parsing}.
    ...
    \Cref{fig:dependency} shows two examples for dependency trees.
...
%    For each word in the sentence and for each constituent where the word is the head, the head of each child constituent is then dependent if it is unequal.
    ...
    They were later brought under the umbrella term \deflab{mildly context-sensitive grammars}, a class that supersedes context-free and is strictly included in context-sensitive grammars.
    ...
    Other research involved parsers that do not rely on grammars at all; for instance parsers modeled as transition systems by \citet{Verseley14,CoaCoh19}, or conditional random fields by \citet{Petrov08}.
    ...
    Some approaches unite grammar-based parsers with discriminative classifiers to combine the advantages of both, i.e.\@ keep parsing procedures interpretable, accurate, and fast.
    ...
    \begin{compactitem}
        \item The automatic extraction from treebanks is notorious for generating large sets of rules, and a great factor for the time needed by grammar-based parsers was identified as the number of rules in the grammar \citep{dunlop2010reducing}.
    ...
    We achieved state-of-the-art scores in the field of discontinuous parsing, not only compared to grammar-based parsers but also to all other approaches that were published at the time \citep{RupMoe21,Rup22}.
    ...
    We start in \cref{sec:preliminaries} with basic notation and definitions used throughout the document; \cref{sec:grammars} recapitulates on the involved grammar formalisms.
    ...
    \begin{compactitem}
        ...
        \item The second approach deals with a generalization of the previous formulation of supertags and their extraction procedure.
            We described this procedure in an earlier publication, but only informally and by means of examples \Citep{Rup22}.
            ...
            In addition to the formalization, the section adds new options for the extraction paramters.
    ...
    The chapter introduces a statistical parsing procedure for the supertags based on definite clause programs in \cref{sec:parsing}; to the best or our knowledge the formalism was not used for parsing natural languages before.
    \Cref{sec:experiments} unrolls the experiments we conducted to assess the presented extraction and parsing procedures.
    A great part of the chapter is dedicated to explore the sets of parameter values and find appropriate configurations for the extraction procedures.
    ...
    \Cref{sec:literature} summarizes related work in the area of constiuent parsing as well as other parsing objectives, specifically syntactic dependencies and semantic representations.
    ...
    Within the summary, we point to some open question that could be resolved in future work.
