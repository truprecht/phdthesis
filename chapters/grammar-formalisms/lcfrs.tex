...
\begin{document}
    ...
    \begin{definition}[Compositions]\label{def:lcfrs:comp}
        ...
        \end{compactenum}
        For each \(k\in \DN\) and \(s, s_1, \ldots, s_k \in \DN_+\), the identifier \(\C^\varSigma_{(s_1\cdots s_k, s)}\) denotes the set of all \abrv{lcfrs} compositions over \(\varSigma\) of length \(s\) where each variable in \(\X_{s_1 \cdots s_k}\) but not other variable in \(\X \setminus \X_{s_1\cdots s_k}\) occurs.
    ...
    \begin{definition}[\abrv{Lcfrs} Rules and Grammars]
        ...
        An \abrv{lcfrs} \deflab<\lcfrs>{rule} (with terminals in \(\varSigma\), nonterminals in \(N\), and fanout \(\mathit{fo}\)) is of the form \(A \to c\,(B_1, \ldots, B_k)\) where \(k \in \DN\), \(A, B_1, \ldots, B_k\) and \(c \in \C^\varSigma_{(\mathit{fo}(B_1) \cdots \mathit{fo}(B_k), \mathit{fo}(A))}\).
    ...
    The length of the compositions occurring in the rules with the same \abrv{lhs} nonterminal are thus equal.

    \begin{definition}[\abrv{Lcfrs} Grammar Forms]We distinguish the rules and the grammars of the following form:
        \begin{compactitem}
...
%    \end{tikzpicture}
    ...
    During the extraction and parsing grammar rules are instantiated with sentence positions. (Cf.\@ \cite[Def. 6.8]{Kal10})
    ...
    \begin{definition}[Constituent Structure for Derivation]
        Consider a position instantiation \((N, [|w|], S, R)\) of some grammar in a word \(w\), and an admissible derivation \(d \in \derivs^R\) of the form \(d = r\,(d_1, \ldots, d_k)\) where the root \(r\) is of the form \(A \to c\,(B_1, \ldots, B_k)\).
        The composition \(c\) contains (possibly empty) sequences of positions \(w_1, \ldots, w_{k+1} \in [|w|]^*\) such that,
        \begin{compactitem}
    ...
    \begin{example}[Continues \cref{ex:lcfrs:comp,ex:lcfrs:rules}]\label{ex:lcfrs:instance}
        Let us consider the \abrv{lcfrs} \(\mathrm{G}\) from the previous examples and an instantiation \(\mathrm{G'} = (N, [8], \nt{v}, R)\) of \(\mathrm{G}\) in the string ``A hearing is scheduled for the issue today''.
    ...
    The following definitions deal with the extraction of \abrv{lcfrs} rules from constituent structures.
    They are similar to the extraction of \emph{treebank grammars} by \citet{MaierSogaard08} but extend the procedure to accommodate lexical \abrv{lcfrs} and can be seen as a reversal of the previous definition for a constituent structure for a derivation.
    ...
    \begin{definition}
        ...
        The families \((s_i \mid i \in [0,k])\) and \((w_i^j \mid i \in [0,k])\) are the fanouts and linearizations of the sets \(\pi_0, \ldots, p_k\), respectively, such that \((w_i^j \mid j \in [s_i])\) idenitifies the linearization for \(\pi_i\).
    ...
    \begin{example}[Linearizations and Compositions, Continues \Cref{ex:lcfrs:instance}]
        ...
        For the successor nodes it is \(w_1^1 = 1\,2\) and \(w_2^1 = 5\,6\), respectively, and their fanout is 1.
        The composition for these tree sets consists (like \(w_0\)) of two components.
        The first is obtained by replacing \(1\,2\) with the variable \(\x_1^1\) in \(w_0^1\) and the second analogously by replacing \(5\,6\) by \(\x_2^1\) in \(w_0^2\).
    ...
    This interpretation is equivalent to the usual probabilistic interpretation \citep[Viterbi semiring]{Goodman99} with logarithmic values.
    ...
    \begin{definition}
        ...
        For each \(n\in \DN_+\), we extended this concept to the sequences of \emph{\(n\)-best parses for \(w\)}.
