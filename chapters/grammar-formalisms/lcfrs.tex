...
\begin{document}
    ...
    \begin{definition}[\abrv{Lcfrs} Grammar Forms]We distinguish rules and grammar instances of the following form:
        \begin{compactitem}
            \item A rule of rank zero, one and two is called \deflab{nullary}, \deflab<unary>{unary grammar rule}[unary] and \deflab<binary>{binary grammar rule}[binary], respectively.
...
%                An \abrv{lcfrs} that contains only lexical nullary rules and non-lexical rules of rank \(\ge 1\) and whose initial nonterminals do not occur in any rule's \abrv{rhs} is called \deflab<\lcfrs>[lcfrs:simple]{simple}.
    ...
    \begin{definition}[Derivation]
        ...
        An \abrv{lcfrs} \deflab<derivation>{\abrv{lcfrs} derivation} is a tree over rules in \(\T_R\) that suffices the following conditions:
        \begin{compactitem}
...
%    \end{tikzpicture}
    ...
    \begin{definition}[Position Instantiation]
        ...
        The \abrv{lcfrs} \(G' = (N, [n], S, R')\) is called the \deflab{position instantiation} of \(G\) in \(w\) if, for each \(r \in R\), the set of rules \(R'\) contains each position instantiation of \(r\) in \(w\).

        A derivation \(d \in \derivs^{R'}\) is called \deflab<admissible>{admissible \abrv{lcfrs} derivation}[admissible], if the yield \(\yield(d)\) of the form \((s_1, \ldots, s_k)\) contains contiguous sequences of positions in \(w\) and the last position in \(s_i\) is at least by 2 smaller than the first position in \(s_{i+1}\) for each \(i \in [k-1]\).
