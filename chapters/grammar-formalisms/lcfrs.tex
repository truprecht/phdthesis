...
\begin{document}
    ...
    The normal form itself, however, is not restrictive: For every \abrv{lcfrs}, there is an equivalent \abrv{lcfrs} in normal form. [\citealp[Lemma~2.2]{SekMatFujKas91}; \citealp[Definition~7.2]{Kal10}]
...
%    \end{tikzpicture}
    ...
    \begin{definition}[Position Instantiation]
        ...
        The grammar \(G' = (N, [n], S, R')\) is called the \deflab[\lcfrs]{position instantiation} of \(G\) in \(w\) if, for each \(r \in R\), the set of rules \(R'\) contains each position instantiation of \(r\) in \(w\).
    ...
    \begin{definition}[Weighted Grammar]
