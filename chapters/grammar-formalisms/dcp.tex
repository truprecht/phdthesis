...
\begin{document}
    ...
    This corresponds to the information flow from the bottom to the top in a derivation as expressed by synthetic attributes (first-order substitution), as well as the information flow from the top to the bottom in a derivation by inherent attributes (second-order substitution) in attribute grammars.
    ...
    \begin{example}\label{ex:dcp:comp}
        ...
        \end{align*}
        The subscript in the family of compositions \(\C\) determines the number of arguments for the functions represented by the elements in \(\C\).
    ...
    \begin{definition}
        Consider a tree \(\xi \in \T_{\varSigma}\) over any alphabet \(\varSigma\) and a collection of \(k \in \DN\) parallel positions \((\rho_i \in \pos(\xi) \mid i \in [k])\) and a common ancestor position \(\rho_0 \unlhd \rho_1, \ldots, \rho_0 \unlhd \rho_k\).
        There is a distinct \abrv{dcp} composition \(c \in \C^\varSigma_k\) such that \(\xi|_{\rho_0} = \sem{c}( \xi|_{\rho_1}, \ldots, \xi|_{\rho_k} )\).
        We denote this composition \(c\) by \(\mathrm{scomp}(\xi, \rho_0, \rho_1, \ldots, \rho_k)\).
    \end{definition}
\end{document}