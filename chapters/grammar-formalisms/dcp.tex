...
\begin{document}
    \section{Definite Clause Programs (\abrv{dcp})}\label{sec:grammar:dcp}
    \abrv{Dcp} were introduced as a formalism to characterize the derivation trees in attribute grammars. \citep{Der85}
    ...
    This corresponds to the information flow from the bottom to the top in a derivation as expressed by synthetic attributes (first-order substitution), as well as the information flow from the top to the bottom in a derivation by inherent attributes (second-order substitution) in attribute grammars. \citep[cf.\@ Section 1: Notations]{Der88}
    ...
    To solve this issue, there were some restrictive forms introduced which exclude circular dependencies in the expressed productions. \citep[Section~3.4 about non-circular attribute grammars]{Cou82}
    ...
    \begin{definition}[Composition]
        ...
        The \(\DN\)-family \((S_k = \{x(v) \mid x \in \X_k, v \in \T_\varGamma(\varSigma \cup Y)^*\} \mid k \in \DN)\) denotes expressions for occurrences of first-order substitutions using \(k\) variables.
    ...
    \begin{example}\label{ex:dcp:comp}
        Consider the following \abrv{dcp} compositions over the alphabets \(\varSigma = \{ \cn{sbar}, \cn{s}, \cn{vp}, \cn{np} \}\) and \(\varGamma = \{\tn{where}, \tn{the}, \tn{survey}, \tn{was}, \tn{carried}, \tn{out}\}\):
        \begin{align*}
            c_1 &= \tn{where} \, \y,
            &c_2 &= \y \, \tn{out},
