...
\begin{document}
    ...
    \begin{compactenum}
        ...
        \item
            ...
            In the following, it extracts an \abrv{lcfrs} derivation of lexical hybrid grammar rules according to the positions assigned to the inner nodes in each subtree.
    \end{compactenum}

    \todo{describe what we understand under lexical grammar, or supertag grammar; in case of lcfrs supertags these are not only plain rules (swapped annotation)}
    ...
    This approach of handling nodes with one successor\todo{introduce terms: rank of a node, unary, binary nodes, and nodes of higher rank} is done after dealing with nodes with more than two successors.
