...
\begin{document}
    ...
    The algorithm is derived from \citet[Algorithm 2, 3 and 4]{HuaChia05} with some adjustments to accomodate our grammar format (i.e.\@ multiple initial nonterminals, non-binary grammars, and the form of \abrv{dcp} supertags).
    ...
    \begin{itemize}
        ...
        \item The collections expressed as families in both \cref{alg:parsing:chart,alg:parsing:deriv} are implemented either as \emph{hash maps} or as \emph{lists} when the index is integerized.
            Even though they are denoted with an initial value for each index in both \cref{alg:parsing:chart,alg:parsing:deriv}, they are empty at start and assume these values implicitly if there is no entry for the index.
