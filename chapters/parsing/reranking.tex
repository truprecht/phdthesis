...
\begin{document}
    ...
    Consider that we extract a disco-dop grammar from a treebank with the set of consituent symbols \(\varGamma\), \abrv{pos} symbols \(\varPi\) and the set of tokens \(\varSigma\).
    ...
    \begin{example}\label{ex:dop}\NoEndMark
        \Cref{fig:ex:dop} shows five rules of a disco-dop grammar that consist of fragments in the constituent tree shown in the left of the illustration.
