...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \item\label{parsing:item:2}
            ...
            If reranking is used, then this step yields a collection of \(k\) best derivation trees, where \(k \in \DN_+\) is a hyperparameter and \emph{best} refers to an endorelation on the weights (e.g.\@ highest probability).
    ...
    The listing of \cref{alg:alg:parsing:deriv} shows how these two structures are used to determine the least parse for the input sentence.
    
    \begin{algorithm}
        \caption{\label{alg:parsing:deriv}
            Reading the best derivation after computing the chart as shown in \cref{alg:parsing:dcp}.
        }
