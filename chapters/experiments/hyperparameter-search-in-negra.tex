...
\bottomrule
    ...
    Aside these two cases, both tables tend to show very close scores in terms of the parsing and supertag prediction accuracy when comparing results with hybrid grammar and \abrv{dcp} supertags within a combination of guide and nonterminal constructors.
    Generally the supertag prediction accuracies are about equal in all cases per model.
    However, in case of the \abrv{dcp} supertags we recorded massive increases in parse time in the columns for the head and dependent guide constructors and both nonterminal constructors with the two extensions.
    Among the groups with same nonterminal and guide constructors, the f1 scores tend to be higher with some configuration of \abrv{dcp} supertags.
    Specifically in case of the pretrained model, \abrv{dcp} supertags with \emph{-nof} extension performed consistently better in the four groups, and \abrv{dcp} supertags with \emph{-disc} extension performed better in the five out of six groups.
    ...
    We continue the experiments with each one configuration for hybrid grammar and \abrv{dcp} supertags, and each the pretrained and supervised model as follows:
    \begin{compactenum}
        ...
        \item the pretrained model with \abrv{lcfrs} or \abrv{dcp} supertags, in both cases with the strict guide and coarse nonterminal constructors, in case of \abrv{dcp} supertags with \emph{-nof} extended nonterminals.
    \end{compactenum}
    ...
    These two parameters were set to fixed values during grid search, we use the four remaining models (the combinations supervised/pretrained model and hybrid grammar/\abrv{dcp} supertags) to see if we can tune these parameters for better results.
    ...
    \begin{itemize}
        \item The first set of experiments is a grid search over values for the two parameters \(k\) and \(\beta\) using each of the four model. During these experiments, we will not use the dop model for the reranking extension, but a gold oracle that selects the constituent tree that maximizes the f-score with respect to the gold constituent tree among the best \(n = 100\) constiuent trees found during parsing. This oracle-f-score shalle give an impression of a potential score that could be achieved using perfect reranking.
        \item The second set of experiement picks a few configurations from the previous step and investigates the results of the parsing process extended by reranking.
    \end{itemize}
    Each experiment predicts supertags for the developement portion of the treebank and parses with them.
    During each of the experiments in this section, we fix the paramter \(n\) to \(100\).
    ...
    However, in contrast to all previous experiments, the parse time in the first set of experiments only involves the time needed to fill the chart during the parsing process, as this is the only part of the parsing process that is actually involved with the two parameters.
...
\bottomrule
    ...
    \begin{table}
        ...
        F1 score for the first (f1) and the best f1 score among the top 100 constituent trees predicted by the parser (of1), parse time ($t$) and amount of sentences that were not able to parse ($\lightning$) for combinations of parameters to the parsing process $\beta$ and $k$ in the developement portion of NeGra. The upper half are results using hybrid grammar supertags, the bottom half are \abrv{dcp} supertags. All supertags were predicted using pretrained models.
        }
        \centering\small
        \setlength{\tabcolsep}{4pt}
        \vspace{.2cm}
...
\bottomrule
    ...
    \Cref{tbl:grid:parsing:supervised,tbl:grid:parsing:pretrained} show the results with for the first set of experiments using the supervised and pretrained models, respectively.
    ...
    \begin{itemize}
        ...
        \item With rising values for $k$, the number of parse fails tend to drop. With larger values of \(\beta\) the number of parse fails tend to increase, due to timeouts during the parsing process.
    \end{itemize}
    ...
    Interestingly, while we could achice higher scores with \abrv{dcp} supertags using the supervised model, the trend is opposite with the pretrained one.
    We pick a few configurations for each model in advance to the experiments with reranking:
    \begin{compactitem}
...
\bottomrule
    ...
    \Cref{tbl:grid:reranking:supervised,tbl:grid:reranking:pretrained} shows the results of the second set of experiments in terms of f1 scores.
    The score of parsing with reranking (r-f1) is set in context of the score without reranking (n-f1) as well as the score with gold oracle reranking (o-f1) from the previous set of experiments.
    The results suggest, that our approach to reranking does in fact slightly increase the parsing accuracy with the supervised model.
    However, it does so quite marginally and comes with additional computational cost; we do not see that it is worth the effort.\footnote{
        The parse time with reranking is not reported in this section as the implementation is quite unoptimized.
        We decided to not spend any more effort due to the underwhelming results.
    }
    In case of the pretrained model, we could not detect any advantage at all, even the opposite is the case.
