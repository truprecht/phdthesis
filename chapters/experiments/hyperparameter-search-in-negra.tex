...
\begin{document}
    ...
    \end{itemize}
    During this grid search, both models are trained for only 10 epochs; we chose this rather small amount of training iterations to conduct the experiments in reasonable time.
...
\bottomrule
    ...
    \end{itemize}
    \Crefrange{tbl:gridsearch:coarse:1}{tbl:gridsearch:coarse:3} show the results in terms of statistics for the extracted supertag blueprints as well as supertag prediction and parsing accuracies in the same dimensions as before.
...
\bottomrule
    ...
    \Cref{tbl:gridsearch:coarse:2,tbl:gridsearch:coarse:3} suggest that the current set of experiments can not improve upon the parsing accuracy achieved by those shown in the previous sections.
    ...
    We propose the reason for this result is as follows:
        ...
        In the case of the head and dependent guides, the set of nonterminals is usually finer than with a strict guide, as we use a directive markers by default.
    ...
    Thus, the sets of extracted blueprints can inherently be seen as \emph{coarser} in the same sense as with nonterminals:
        If we fix every other hyperparameter and compare \abrv{dcp} supertag blueprints with hybrid grammar supertag blueprints, then the former defines a partition of the latter; that is, for each hybrid grammar blueprint, there is exactly one \abrv{dcp} blueprint that just differs in the missing composition.
...
\bottomrule
    ...
    The lines marked with ``(\abrv{hg})'' above the results for each nonterminal constructor show the previous experiments with hybrid grammar supertag blueprints for comparison.
    ...
    \begin{table}
        ...
        The parsing f1-score (f1), accuracy of predicted supertag blueprints (acc.), parse time ($t$), and number of parse fails ($\lightning$) using the supervised prediction model in \negra{}'s development set with extracted \abrv{dcp} supertags for configurations of guides and nonterminal constructors. The two rows marked with ``(\abrv{hg})'' show values obtained with hybrid grammar supertags for comparison.
        }
        \centering
        \setlength{\tabcolsep}{5pt}
        \vspace{.2cm}
...
\bottomrule
    ...
    \Cref{tbl:gridsearch:dcp:2,tbl:gridsearch:dcp:3} show the scores involved in the supertag prediction and in parsing with the extracted supertag blueprints.
    ...
    Specifically, in the case of the pre-trained model, \abrv{dcp} supertags with nof extension performed consistently better in the four groups, and \abrv{dcp} supertags with disc extension performed better in five out of six groups.
    ...
    \begin{itemize}
        \item the supervised model with \abrv{hg} or \abrv{dcp} supertags, in both cases with the strict guide and classic nonterminal constructors (without extensions),
        \item the pre-trained model with \abrv{hg} or \abrv{dcp} supertags, in both cases with the strict guide and coarse nonterminal constructors, in case of \abrv{dcp} supertags with nof extended nonterminals.
    \end{itemize}
...
\bottomrule
    ...
    Interestingly, while we could achieve higher scores with \abrv{dcp} supertags using the supervised model, the trend is opposite with the pre-trained one.
    ...
    \begin{itemize}
        ...
        \item for the pre-trained model, we fix \abrv{hg} supertags with \(k = 20\) and \(\beta \in \{0.1,0.5,2,5\}\).
    \end{itemize}
...
\bottomrule
    ...
    \begin{itemize}
        ...
        \item the pre-trained model to \abrv{hg} supertags with parsing parameters \(k=20\) and \(\beta=0.1\).
    \end{itemize}
\end{document}