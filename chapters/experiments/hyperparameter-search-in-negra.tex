...
\begin{document}
    ...
    \begin{compactitem}
        \item A pretrained architecture model utilizing a \abrv{bert} model as embedding \footnote{
    ...
    \begin{table}
        \caption{\label{tbl:gridsearch:1:3}
            Parsing f1 score, accuracy of predicted supertag blueprints and number of parse fails using the pretrained architecture model in the \negra{}'s development set for combinations of guide and nonterminal constructors.
        }
        \centering
        \vspace{.2cm}
        \begin{tabular}{l|rrr|rrr|rrr|rrr}
...
\bottomrule
    ...
    \begin{table}
        ...
        Parsing f1 score, accuracy of predicted supertag blueprints and number of parse fails using the pretrained architecturen model in \negra{}'s development set with extacted \abrv{dcp} supertags for configuations of guides and nonterminal constructors. The two rows marked with ``(hg)'' show values obtained with hybrid grammar supertags for comparison.
        }
        \centering
        \setlength{\tabcolsep}{5pt}
        \vspace{.2cm}
...
\bottomrule
    ...
    Specifically in the case of the pretrained model, \abrv{dcp} supertags with nof extension performed consistently better in the four groups, and \abrv{dcp} supertags with disc extension performed better in the five out of six groups.
