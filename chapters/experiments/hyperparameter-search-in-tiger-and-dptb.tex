...
\begin{document}
    ...
    We explore the remaining grid of configurations thoroughly with hybrid grammar and \abrv{dcp} supertags, which yields a total of $3 \cdot 2 \cdot 2 = 12$ experiments per treebank and prediction model with \abrv{lcfrs} supertags and 2--4 additional experiments with \abrv{dcp} supertags per model.
    ...
    \begin{table}
        ...
        The number of extracted hybrid grammar supertag blueprints from the training portion ($N$) and the number of extracted supertag blueprints in the dev portion that do not appear in the training portion ($\overline{N}$) using the \dptb{} treebank.
        }
        \centering
 ...
 nont.   &\(v\)   & $N$ & $\overline{N}$ & $N$ & $\overline{N}$ & $N$ & $\overline{N}$ \\ \hline
  ...
  coarse  & \(2\)  & 3780 &  44 &  6853 &  80 &  7030 & 108  \\
    ...
    \begin{table}
        ...
        The parsing f1 score, the accuracy of predicted hybrid grammar supertag blueprints, parse time ($t$), and number of parse fails ($\lightning$) in \abrv{dptb}'s development set for both prediction models (Mod.), combinations of guide and nonterminal constructors as well as vertical Markovization context ($v$) for the used rank transformation.
        }
        \centering
        \setlength{\tabcolsep}{4pt}
        \vspace{.2cm}
...
sup. & coarse  & \(2\)  & 89.97 & 0.82 & 15.01 & 5 & 88.87 & 0.80 & 15.95 & 38 & 89.76 & 0.82 & 15.15 & 15 \\
    ...
    \begin{table}
        ...
        The number of extracted supertag blueprints ($N$), The number of supertag blueprints that appear in the development but not in the training portion, ($\overline{N}$), The parsing f1 score, the accuracy of predicted \abrv{dcp} supertag blueprints, parse time ($t$) and The number of parse fails ($\lightning$) in \abrv{dptb}'s development set for both prediction models (Mod.), combinations of guide and nonterminal constructors as well as vertical Markovization context ($v$) for the used rank transformation.
        }
        \centering
        \setlength{\tabcolsep}{4pt}
        \vspace{.2cm}
...
sup. &   5  & 90.71 & 2.6 & 9 & 90.55 & 5.4 & 13 & 90.56 & 7.0 & 14 \\
  ...
  nont.   &\(v\)   & $N$ & $\overline{N}$ & $N$ & $\overline{N}$ & $N$ & $\overline{N}$ \\ \hline
   ...
   coarse  & \(2\)  & 5197 & 116 &  9896 & 296 & 10652 & 377  \\
    ...
    \begin{compactitem}
        \item strict guide and coarse nonterminal constructors, no added vertival Markovization context ($v=1$) and hybrid grammar supertags for the pre-trained model, and
    ...
    \begin{table}
        ...
        of extracted supertag blueprints ($N$), of supertag blueprints that appear in the development but not in the training portion, ($\overline{N}$), The parsing f1 score, the accuracy of predicted \abrv{dcp} supertag blueprints, parse time ($t$) and parse fails ($\lightning$) in \abrv{dptb}'s development set for both prediction models (Mod.), combinations of guide and nonterminal constructors as well as vertical Markovization context ($v$) for the used rank transformation.
        }
        \centering
        \setlength{\tabcolsep}{4pt}
        \vspace{.2cm}
