...
\begin{document}
    ...
    An extension for the latter uses a table to partition the set of constituent symbols for the construction of coarse nonterminals.
    ...
    Generally, when training classifiers with \abrv{ann}, there are numerous hyperparameters for several aspects in the architectures as well as parameters and extensions for the gradient descent algorithm.
    Integrating these parameters into the grid search for the values in the extraction procedure would result in a vast search space.
    For most of our experiments, we will use fixed values to ensure we can conduct the experiments reasonable time.
    We use the following sources to fix the values for the training parameters:
    \begin{compactitem}
    ...
    We use the following parameters for the training the \abrv{ann} models:
    \begin{itemize}
    ...
    Most parameters involved in fine-tuning the pretrained model (optimizer, learning rate, number of epochs, weight decay) are filled with advised values by \citet{Devlin2019}.
    For the supervised model, we orientate towards the parameter values in the experiments by \citet{Cor20} and \citet{StaSte20}, as they used the same architecture.
    Generally, we use the same parameter values as in earlier experiments \citep{Rup22}.
\end{document}