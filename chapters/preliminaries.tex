...
\begin{document}
    ...
    In case \(A\) and \(B\) coincide, we call \(R\) an \deflab{endorelation} on \(A\).
    ...
    We call this family a \deflab{parametrized function}[$\varTheta$-parametrized function]; it is denoted in the usual function notation \(f_\theta\colon A \to B\).
    ...
    An \(A\)-indexed \deflab{partition} of \(B\) is a \(A\)-indexed family \(p\) over subsets in \(\power(B)\) such that:
    \begin{inparaenum}
    ...
    For each \(e \in [|w|]\), the subsequence \(w_{:e}\) is called \deflab<prefix>{proper prefix}[proper prefix in \(w\)]; analogously, for each \(s\in [1,|w|+1]\), the subsquence \(w_{s:}\) is a \deflab<postfix>{proper postfix}[proper postfix in \(w\)].

    Let \(\unlhd\) be a total order on \(A\), the \deflab{lexicographic order} on \(A^*\), denoted by \(\unlhd^*\), is defined such that
    \begin{compactenum}
    ...
    We call the set \(I\) \deflab<contiguous>{contiguous set}[contiguous], if there is a least element \(\mathit{min} = \min I\) and a geatest element \(\mathit{max} = \max I\) such that \(I = [\mathit{min}, \mathit{max}]\) includes exactly the integers in the interval between them.
    A \deflab<contiguous>{contiguous sequence} is a non-empty sequence over \(\DZ\) of some length \(n \DN_+\) of the form \(i (i+1) (i+2) \ldots (i+n)\); that is, the leftmost element \(i\) is the least, and the rightmost element \(i+n\) is the greatest in the sequence, and the elements between them increase by one at each position.
    ...
    A \deflab{node}[node in the tree \(t \in \T_\varSigma(\varGamma)\)] is an occurrence of a symbol in \(\varSigma \cup \varGamma\); it is a combination of the symbol \(\sigma\) and a position it occurs.
    If a node is accompanied by a sequence of trees denoted in parentheses behind the symbol, these trees are called the \deflab{child}[children] of the node.
    ...
    Each other node within the parenthesis is called a \deflab{sibling}.
    The leftmost node is called the \deflab{root node}[root] of the tree; each node without children is called a \deflab{leaf node}[leaf] of the tree; and each node with children is called an \deflab{inner node}.
    The sequence of all sybols at leaves in a tree \(t\) from left to right is called the \deflab{yield}[yield of \(t\)] and denoted by \(\yield(t)\).
    The \deflab<rank>{rank of a node} is its number of children, \(\rank(t)\) denotes the rank of the root in \(t\).
    ...
    A \deflab[position]{position in a tree}[position in \(t\)] is a sequence $w \in \DN^*$ that identifies a node in the tree.
    ...
    Each subsequence in \(t\) that is also a tree in \(T_\varSigma\) is called a \deflab{subtree}[subtree in \(t\)].
    ...
    Two positions \(\rho, \rho' \in \pos(t)\) are called \deflab{independent positions}[independent], if \(\rho\) is neither a descendant nor an ancestor of \(\rho'\).

    A \deflab{subforest} of \(t\) is a sequence of trees occurring consecutively (as siblings) within \(t\).
...
%    \end{proof}
    ...
    An \deflab{indexed tree}[indexed tree over \(\varSigma\)] is an element in \(\T_{\varSigma}(\DN_+)\); that is, inner nodes are symbols in \(\varSigma\), and leaves are positive integers, such that each leaf occurs at most once.
    ...
    A (first-order) \deflab<substitution>{first-order substitution}[substitution of \(X\) in \(c\)] defined by an \(X\)-indexed family \((v_x \in \varSigma^* \mid x \in X)\) is denoted by \(c[v]\) and yields the string in \(\varSigma^*\) obtained from \(c\) by replacing every occurrence of symbols \(x \in X\) by \(v_x\).
    ...
    In the syntactic analysis of sentences in natural languages, we deal with \deflabs{constituent tree}.
    They illustrate a hierarchy of sub-phrases in the sentence, where a category labels each element in the hierarchy, called \deflab{constituent symbol}.
    ...
    They are mutually exclusive to the constituent symbols found in upper layers, i.e., each symbol occurring at the first layer must be a \abrv{pos} tag and no other constituent symbol, and each symbol above the first layer must not be a \abrv{pos} tag.
    ...
    We assume a set \(\Sigma\) of words, punctuation marks, parentheses, etc.\@ that are used to form sentences; we call the elements in \(\Sigma\) \deflabs{token}.
    ...
    In each constituent tree, we distinguish three parts: the sequence of tokens, the \abrv{pos} tag above each token, and the \deflab{constituent structure} that contains the constituent symbols and defines the hierarchical structure.
    ...
    A \deflab{head asssignment}[head assignment for $(t,p,w)$] is a function \(h\colon \pos(t) \to \DN_+\) assigning each inner node position in the constituent structure \(t\) to one of the leaves at or below it.
    ...
    \]
    The \deflab<head asssignment>{lexical head assignment}[lexical head assignment for $(t,p,w)$] is such a head assignment.
    ...
    The leaf \(\head_{(t,p,w)}(\rho)\) is called the \deflab{lexical head} of the node at \(\rho\).
    ...
    \end{inparaenum}
    For both, there is a treebank that is reserved for the evaluation (called \emph{test portion}).
    ...
    The \emph{evalb}-style labeled \deflab{f1-score} \citep{Black91,Col97} was established as the standard for measuring the accuracy of predicted constituent structures.
    ...
    A \deflab{model}[(prediction) model] is a specification that defines how each input is mapped to a label.
    Each model chooses a label that optimizes a \deflab{confidence} value (e.g., a score or probability), so we view each model as a function that computes this value for each input and label.
    ...
    \deflab{long short-term memory}[Long short-term memory] \defabrv{long short-term memory}{\abrv{lstm}} cells \citep{Hoc97} were established as a quasi-standard for these inherent modules.
    ...
    The \deflab{transformer} architecture \citep{vaswani2017attention} was established as an alternative to recurrent neural networks for sequence processing.
...
%    There are several extensions to the baseline training process that aim to accelerate or stabilize the training process or improve the prediction quality; we use the following:
    ...
    Usually, they are \emph{generic}; that is, they are designed to be used in many different tasks. In that context, such a specific task is called \deflab{downstream}.
