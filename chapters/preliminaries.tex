...
\begin{document}
    ...
    A \deflab{node in a tree}[node in the tree \(t \in \T_\varSigma(\varGamma)\)] is an occurrence of a symbol in \(\varSigma \cup \varGamma\) within \(t\); it is a combination of the symbol \(\sigma\) and the position it occurs.
    If a node is accompanied by a sequence of trees denoted in parentheses behind the symbol, these trees are called the \deflab{child trees/nodes}[children] or \deflab{successor trees/nodes}[successors] of the node.
