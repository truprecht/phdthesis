...
\begin{document}
    ...
    \begin{inparaenum}
        ...
        \item for each symbol \(\sigma \in \varSigma\), positive integer \(k \in \DN_+\) and trees \(t_1, \ldots, t_k \in T\), the sequence \(\sigma(t_1 \cdots t_k)\) is in \(T\).
    \end{inparaenum}
    ...
    The \deflab<tree>{ancestor}[ancestors] of \(\rho\) are all real prefixes of the sequence \(\rho\), denoted by \(\ancestors(\rho) = \{ \rho_{:e} \mid e \in [|\rho|] \}\).
...
%    \end{proof}
    ...
    An \deflab<tree>[itree]{indexed tree} over \(\varSigma\) is an element in \(\T_{\varSigma}(\DN_+)\), i.e.\@ inner nodes are symbols in \(\varSigma\) and leaves are positive integers, such that each leave occurs at most once.
    ...
    For further elaborations in the field of neural networks for natural language processing, consult \citet{Gol22}.
    ...
    The most simple form of embeddings is \deflab<>{static embeddings}.
...
%    A prominent example of such an objective is \emph{cross entropy loss} which is often used when the confidence values for the labels are interpreted as a probability distribution.
...
%    During the training process, the value is computed for a small portion of the data (called \emph{batch}) and the parameters are updated using a partial derivative of the value with respect to the weight.
...
%    There are several extensions to the baseline training process that aim to accelerate or stabilize the training process or improve the prediction quality; we use the following:
    ...
    \begin{enumerate}
        ...
        \item
            The second architecture consists of a pre-trained \abrv{llm} that is topped by one feed-forward layer (this design is pretty common and recommended for classification predictions using \abrv{llm} embeddings e.g. by \citet{Devlin2019}).
            If we consider the \abrv{llm} as a black box, it is conceptually extremely simple yet effective, but its strength relies of course upon the complex design and pretraining of the \abrv{llm}.
