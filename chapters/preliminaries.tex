...
\begin{document}
    ...
    A \deflab<empirical prediction>{model}[(prediction) model] is a specification that defines how each intput is mapped to a label.
    Each model chooses a label that optimizes a \deflab<empirical prediction>{confidence} value (e.g. a score or probability), so we view each model as a function that computes a confidence for each input and label.
    ...
    Hyper parameters determine details about the model and a training process. %, to some extend, the model and the training objective can also be seen as hyper parameters.
    ...
    \begin{example}[Statistical parsing in the empirical prediction framework.]
        ...
        The domain of inputs is the set of sequences over tokens (that preferably coincide with german words) and the domain of labels is the set of constituent trees where the constituent and \abrv{pos} symbols are defined by the NeGra corpus.
    ...
    As mentioned in the last example, we use neural networks to implement the prediction of a sample of supertags for each position input sentences.
    ...
    As in the parent section, we will only give a brief overview for the concepts needed to understand the implementations discussed in this thesis.
...
%    Such an objective is a function that rates the prediction for each example in comparison to the ground truth and accumulates the results in the whole set.
%    A prominent example for such an objective is \emph{cross entropy loss} that is often used when the confidence values for the labels are interpreted as probability distribution.
%    It is the sum of negative logarithms over all confidence values; the term \emph{loss} indicates that the value should be minimized during the training process.
%    During the training process, the value is computed for small portion of the data (called \emph{batch}) and the parameters are updated using a partial derivative of the value with respect to the weight.
%    These updates are scaled by a (typically small) real value, called \emph{learning rate}, and repeated a fixed number of times or until a certain criterion is met.
%    There are several extensions to the baseline training process that aim to accelerate or stabilize the training process or improve the prediction quality; sthat we use the following:
    ...
    \begin{itemize}
        \item
            \abrv{Sgd} is an iterative process and usually each iteration is performed using a small amount of data, called \emph{batch}.
        ...
        \item
            There are some measures that aim to improve the ability to generalize from training examples, among them are \emph{weight decay} and \emph{dropout} \citep{dropout}.
    ...
    \begin{compactitem}
        ...
        \item
            Large language models (short \abrv{llm}) are, as the name suggests, not only single modules but large architectures for sequence processing.
    ...
    \begin{enumerate}
        \item
            ...
            It contains two modules for embedding, a static word embedding and character embeddings, which are followed by some layers of bidirectional recurrent modules utilizing \abrv{lstm}, and finally an \abrv{mlp}; \cref{fig:architecture:supervised} shows an illustration.
            It does not rely on any pretrained modules and therefore implements supervised learning for the prediction of supertags.
        \item
            The second architecture consists of a pretrained \abrv{llm} that is topped by one feed-forward layer (this design is pretty common and recommended for classification predictions using \abrv{llm} embeddings e.g. by \citet{Devlin2019}).
            If we consider the \abrv{llm} as a black box, it is conceptually extremely simple yet effective, but its strength relies of course in the complex design and pretraining of the \abrv{llm}.
            As noted before, using such a pretrained model is considered semi-supervised learning for the prediction of supertags.
    \end{enumerate}
    ...
    \begin{figure}
        ...
        \caption{\label{fig:architecture:supervised}
            ...
            The input sentence (bottom) is embedded using static word embeddings and character-based embeddings; the latter are illustrated to the right and consist of static character embeddings which are fed into a bidirectional \abrv{lstm}.
            The embeddings are topped by bidirectional \abrv{lstm} layers (in this illustration 2) and a linear layer such that the size of the input is the number of supertag blueprints.
        }
    \end{figure}
