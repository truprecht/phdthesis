...
\begin{document}
    ...
    For each \(e \in [|w|]\), the subsequence \(w_{:e}\) is called \deflab<prefix>{proper prefix}[proper prefix in \(w\)]; analogously, for each \(s\in [1,|w|+1]\), the subsequence \(w_{s:}\) is a \deflab<postfix>{proper postfix}[proper postfix in \(w\)].
    ...
    A \deflab{node}[node in the tree \(t \in \T_\varSigma(\varGamma)\)] is an occurrence of a symbol in \(\varSigma \cup \varGamma\) within \(t\); it is a combination of the symbol \(\sigma\) and the position it occurs.
...
%    \end{proof}
    ...
    A (first-order) \deflab<substitution>{first-order substitution}[substitution of \(X\) in \(c\)] is defined by an \(X\)-indexed family \((v_x \in \varSigma^* \mid x \in X)\), denoted by \(c[v]\), and yields the string in \(\varSigma^*\) obtained from \(c\) by replacing every occurrence of symbols \(x \in X\) by \(v_x\).
    ...
    \end{compactitem}
    In such a triple, we distinguish three parts of each constituent tree: its sequence of tokens \(w\), the \abrv{pos} tag for each token in \(p\), and the \deflab{constituent structure} \(t\) that contains the constituent symbols and defines the hierarchical structure.
    ...
    A \deflab{head assignment}[head assignment for a constituent tree $(t,p,w)$] is a function \(h\colon \pos(t) \to \DN_+\) assigning each position in the constituent structure \(t\) to one of the leaves at or below it.
    ...
    \]
    The \deflab<head assignment>{lexical head assignment}[lexical head assignment for $(t,p,w)$] is such a head assignment.
    ...
    Each model chooses a label that optimizes a \deflab{confidence} value (e.g., a score or probability) for a given input, so we view each model as a function that computes this value for each input and label.
