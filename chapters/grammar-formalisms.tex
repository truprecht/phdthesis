...
\begin{document}
    ...
    Within the scope of this thesis, however, grammars are not used to compute strings, but the opposite:
        We analyze strings in consideration which collection of grammar rules may be used to derive it; this is called \emph{parsing}.
    ...
    Grammars for natural languages are usually highly ambiguous, i.e.\@ there are many different possible combinations of rules that derive a given string.
    One approach to handle such ambiguities are weight assignments, which are used to rank parses and choose the supposed best among them.
    ...
    In the literature, there is lots of theoretical work about general frameworks for weight structures in parsing with grammars.
    E.g., \citet{Goodman} gives an overview over weighted parsing with weights withing the semiring framework.
    ...
    Secondly, \cref{sec:grammar:dcp} aims at definite clause programs (\abrv{dcp}), which were introduced by \cite{Der85} as a logic characterization of attribute grammars.
    They were used in the context of parsing by \cite{Ned14,Geb17,Geb20} in combination with \abrv{lcfrs} forming hybrid grammars.
    ...
    Parsing with \abrv{dcp} finds a derivation that produces a tree with the input sentence in its leaves and assume the tree as constituency structure.

    Lastly, \cref{sec:grammar:hybrid} considers \abrv{lcfrs}/\abrv{dcp} hybrid grammars as introduced by \citet{Ned14}.
