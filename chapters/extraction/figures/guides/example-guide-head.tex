...
\begin{document}
    \begin{tikzpicture}[
        ...
        \begin{scope}[every node/.style={font=\small, inner sep=1pt}]
            \node[above=18ex of pos, xshift=-3em] (sbar) {\cn{sbar}}
            child {node (s) {\cn{s}}
        ...
        % shortest guide
        \begin{scope}[font=\small, gray, every node/.style={inner sep=1pt, circle, font=\scriptsize}]
            \node [right=of sbar, draw] {4};
            \node [right=of s] {4};
            \node [right=of vp1] {4};
            \node [right=of np, draw] {3};
            \node [right=of vp2, draw] {5};
            \node[draw, gray, circle,above= of pos-1-1] {\phantom{1}};
            \node[draw, gray, circle,above= of pos-1-2] {\phantom{1}};
            \node[draw, gray, circle,above= of pos-1-6] {\phantom{1}};
