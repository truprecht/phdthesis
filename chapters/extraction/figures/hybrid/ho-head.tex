...
\begin{document}
    \begin{tikzpicture}[
        ...
        \begin{scope}[every node/.style={font=\small, inner sep=1pt}]
            \node[above=14ex of pos, xshift=-3em] (sbar) {\cn{sbar}+\cn{s}}
            child { node[xshift=3em] (vp1) {\cn{vp}}
                child { node[xshift=2em] (vp2) {\cn{vp}}
                    child { node[above= of pos-1-1] (term0) {1} }
                    child { node[xshift=13.5ex] (vpbin) {\cn{vp}\binsym{}}
                        child { node[above=of pos-1-5] (term4) {5} }
                        child { node[above=of pos-1-6] (prt) {6}    edge from parent[double] }  edge from parent[double]}}
                child {
                    node[above=of pos-1-4] (term3) {4}  edge from parent[double]}
                edge from parent[double]}
            child { node[yshift=-7ex, xshift=-2.5em] (np) {\cn{np}}
                child { node[above=of pos-1-2] (term1) {2} }
                child {
                    node[above=of pos-1-3] (term2) {3} edge from parent[double]}};
        \end{scope}

        % shortest guide
        \begin{scope}[font=\small, gray, every node/.style={inner sep=1pt, circle, draw, font=\scriptsize}]
            \node [right=of sbar] {4};
            \node [right=of vp1, draw=none] {4};
            \node [right=of np] {3};
            \node [right=of vp2] {5};
            \node [right=of vpbin, draw=none] {5};
            \node[draw, gray, circle] at (term0) {\phantom{3}};
            \node[draw, gray, circle] at (term1) {\phantom{3}};
            \node[draw, gray, circle] at (prt) {\phantom{3}};
        \end{scope}
