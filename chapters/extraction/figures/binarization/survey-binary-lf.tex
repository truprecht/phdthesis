...
\begin{document}
    \begin{tikzpicture}[
        ...
        \matrix (pos) [matrix of nodes, every node/.style={anchor=center, inner sep=0pt}, column sep=2ex] {
            \cn{wrb} & \cn{pt} & \cn{nn} & \cn{vbd} & \cn{vbn} & \cn{prt+rp} \\
        };

        \begin{scope}[every node/.style={font=\small, inner sep=1pt}]
            \node[above=17ex of pos, xshift=-10mm] (sbar) {\cn{sbar}+\cn{s}}
            child { node[xshift=3em] (vp1) {\cn{vp}}
                child { node[xshift=2em] (vp2) {\cn{vp}}
                    child {node[xshift=1em] (vpbin) {\cn{vp}\binsym{\cn{wrb},\cn{vbn}}}
            ...
            child { node[yshift=-10ex, xshift=-3.8em] (np) {\cn{np}}
                child { node[above=of pos-1-2] (term1) {2} }
                child { node[above=of pos-1-3] (term2) {3}}};
        \end{scope}
