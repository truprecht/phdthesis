\documentclass[../../../../document.tex]{subfiles}

\begin{document}
    \begin{tikzpicture}[
            edge from parent path={(\tikzparentnode.south) -- ++(0,-.5ex) -| (\tikzchildnode.north)},
            every path/.style={line width=.2ex},
            level distance=4ex,
            anchor=center,
            every node/.style={font=\small},
            baseline=(np.base)
        ]
        \matrix (pos) [matrix of nodes, every node/.style={anchor=center, inner sep=0pt}, column sep=4ex, row sep=1mm] {
            \cn{cc} & \cn{rb} & \cn{md} & \cn{prp} & \cn{nn} & \cn{.}\vphantom{\cn{prp}} \\
            \tn{And}\strut & \tn{neither}\strut & \tn{can}\strut & \tn{your}\strut & \tn{pilot}\strut & \tn{.}\strut \\
        };

        \node[above=27.5mm of pos-1-3] {\cn{s}}
        [sibling distance=6em]
        child {
            node {\cn{s}\binsyml{}}
            child {node[above=1mm of pos-1-1] {1}}
            child { node[above=14mm of pos-1-2] {\cn{s}\binsymr{}}
                child {
                    node {\cn{advp}}
                    child {
                        node[above=1mm of pos-1-2] {2}
                        edge from parent[double]}
                    edge from parent[double]}
                edge from parent[double]}}
        child {
            node[above=1mm of pos-1-3] {3}
            edge from parent[double]}
        child {
            node {\cn{s}\binsymr{}}
            [sibling distance=3em]
            child {node (np) {\cn{np}}
                child {node[above=1mm of pos-1-4] {4}}
                child {
                    node[above=1mm of pos-1-5] {5}
                    edge from parent[double]}
                edge from parent[double]}
            child {
                node[above=14mm of pos-1-6] {\cn{s}\binsymr{}}
                child {
                    node[above=1mm of pos-1-6] {6}
                    edge from parent[double]}}};

        % redraw corssing edges with white foundation
        %        \begin{scope}
            %            \draw[white, line width=1ex] (vp2.south) ++(-.5ex,-.5ex) -| (term4.north);
            %            \draw (vp2.south) -- ++(0,-.5ex) -| (npr.north);
            %            \draw (vp2.south) -- ++(0,-.5ex) -| (np1.north);
            %            \draw[double] (vp2.south) -- ++(0,-.5ex) -| (term4.north);
            %
            %            \draw[white, line width=1ex] (vp1.south) -- ++(0,-.5ex) -| (term3.north);
            %            \draw (vp1.south) -- ++(0,-.5ex) -| (vp2.north);
            %            \draw[double] (vp1.south) -- ++(0,-.5ex) -| (term3.north);
            %        \end{scope}
    \end{tikzpicture}
\end{document}
