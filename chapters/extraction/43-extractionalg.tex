...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item
            ...
            \begin{inparaenum}
                ...
                \item \(\rho_1, \ldots, \rho_s\) are ordered concerning the least leaf in \(L(\rho_1), \ldots, L(\rho_s)\).
            \end{inparaenum}
    ...
    The topmost descendent positions below \(\varepsilon\) that \(\varphi^{\text{top}}\) assigns to other leaves are \(\rho_1 = 1\,1\,1\) (the bottom \nt{vp} node) and \(\rho_2 = 1\,2\) (the \nt{np} node).
    The \abrv{lhs} \(A_0\) and \abrv{rhs} nonterminals \(A_1, A_2\) are determined from the nodes at these three positions, respectively, as well as the fanouts of the three sets \(L(\rho_0) = [6]\), \(L_(\rho_1) = \{1,5,6\}\) and \(L(\rho_2) = \{2,3\}\); they are \(A_0 = \nt{sbar}_1\), \(A_1 = \nt{vp}_2\) and \(A_2 = \nt{np}_1\).
    ...
    The \abrv{dcp} composition is constructed from the nodes in \(\xi\) between \(\rho_0\) and its descendents \(\rho_1\) and \(\rho_2\), it is \(c_2 = \mathrm{scomp}(\xi, (\varepsilon, [6]), (1\,1\,1, \{1,5,6\}), (1\,2, \{2,3\})) = \cn{sbar}(\cn{s}(\cn{vp}(\x_1(\varepsilon), 4), \x_2(\varepsilon)))\,\y\).
