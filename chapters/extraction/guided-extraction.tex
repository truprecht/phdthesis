...
%        \mathrm{strict}(\xi)(\rho) = \min \; \yield(\xi|_{\rho\cdot 2})
    ...
    The partition \(\mathit{fo}\) and its connection to the definition of each nonterminal constructors assure that the constructed nonterminals are considered with consistent fanouts.
...
%    For our examples, the nonterminals are \(\text{S}\) and \(\text{V}\).
    ...
    \subsection{Lexical Derivation Extraction}\label{sec:extraction:hg:alg}
...
%        \item \(\phi^{\text{top}}(\rho) = \{\phi(\rho)\} \setminus \{\phi(\rho') \mid \rho' \in \ancestors(\rho)\}\) assigns each position \(\rho \in \pos(\xi)\) to a unary set
    ...
    All consecutive nodes that are assigned to \(i\) as well are condensed into the same rule.
    ...
    \begin{itemize}
        ...
        \item the \abrv{dcp} projection \(\pi_{\abrv{dcp}}(d)\) is a derivation for the tree \(\xi|_\rho\) (except if there is a leaf in \(\yield(\xi|_\rho) \setminus L(\rho)\), then it is replaced by \(\y\)).\todo{reformulate}
    \end{itemize}
    ...
    \(L\) maps each position to the encircled values that are illustrated at or next to descendant positions.
    ...
    This example shows how the extraction procedure does not preserve the structure from the binarization in case of the head guide.
    When all nodes that are formed during binarization are assigned to the same leaf, then they are part of the same rule.
    The resulting derivation is equivalent to the extraction of the plain (unbinarized) constituent structure, as shown in the previous example.
    \todo[inline]{anderes Beispiel suchen}
    \begin{center}
