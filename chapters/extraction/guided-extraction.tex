...
%        \mathrm{strict}(\xi)(\rho) = \min \; \yield(\xi|_{\rho\cdot 2})
    ...
    \begin{itemize}
        \item Each constituent structure is interpreted as dependency tree by combining the nodes that are assigned to the same lexical head. This will yield a tree consisting of sentence positions, each occurring exactly once.
    ...
    \end{itemize}
    \Cref{sec:extractionalg} shows an example for the extraction of a derivation using the head guide.
...
%    For our examples, the nonterminals are \(\text{S}\) and \(\text{V}\).
    ...
    \begin{itemize}
        ...
        \item The \emph{disc extension} removes the fanout subscript if it is 1, and replaces it by the marker ``$_{\text{disc}}$'' if it is greater then 1; i.e.\@ it distinguishes nonterminals for continuous and discontinuous derivations.
    \end{itemize}
    in \cref{sec:experiments} will refer to the nonterminal constructors extended by adding the name as suffix, for instance \emph{coarse-disc} for the coarse nonterminal constructor with the disc extension.
...
%        \item \(\phi^{\text{top}}(\rho) = \{\phi(\rho)\} \setminus \{\phi(\rho') \mid \rho' \in \ancestors(\rho)\}\) assigns each position \(\rho \in \pos(\xi)\) to a unary set
    ...
    The dependent guide constructor expects a binary constituent structure complemented by a head assignment; thus, it can only be coupled it with the head-outward binarization strategy.
    ...
    Formally, the head-outward binarization strategy produces viable inputs for the head guide constructor, but the canonical head assignments are not useful when it comes to the extraction:
        A head guide will combine all artificial nodes that are introduced during the binarization and assigned to the same lexical head; hence the effects of the rank transformation are void.
    ...
    Consider the constituent structureafter applying the head-outward binarization strategy from the previous example  together with the values assigned by its head guide.
    All nodes that originate from the \cn{s} constituent are assigned to the lexical head \(3\).
    For the root position $\rho_0 = \varepsilon$, there are four descendant positions that are assigned to different lexical heads: $\rho_1 = 1\,1\,1$, $\rho_2=1\,1\,2$, $\rho_3 =1\,2$ and $\rho_4=2$.
    ...
    The following illistration shows the resulting derivation.
    \begin{center}
        \subfile{figures/hybrid/pilot-ho-head-deriv.tex}
    \end{center}
    \exampleqed

    Aside from these restrictions of compatible binarization strategies and guide constructors, the nonterminal constructors and Markovization paramters can be chosen freely.
