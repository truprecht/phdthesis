...
\begin{document}
    ...
    In this section, we focus on a generalization of the extraction of lexical grammar rules as introduced by \citet{Rup22} that were investigated to tackle these two issues.
    ...
    \end{compactitem}
    To implement the two generalizations, the extraction described in this section will produce \abrv{lcfrs}/\abrv{dcp} hybrid grammar rules.
    As in the previous section, this extraction process may be characterized by four consecutive steps:
    \begin{enumerate}
        \item\label{extraction:hg:ranktrans}
            The tree is processed by a compatible rank transformation.
            For some guide constructors, applying no rank transformation at all may be a choice as well.
    ...
    \end{enumerate}
    The first two steps are explained in the following \cref{sec:extraction:guided:guide}, which defines the concept of guide constructors, gives examples, and explains which rank transformations are sensible options.
    The concept of nonterminal constructors and some examples are discussed in \cref{sec:extraction:guided:nt}; the remainder of \cref{extraction:hg:alg} is explained in \cref{sec:extraction:guided:alg}.
    ...
    \todo[inline]{
        The concept can be generalized further: decouple the guides into two, a lexical decomposition for the string grammar derivation and a guide that constructs the tree grammar.
    ...
    \begin{definition}[Guide]
        ...
        \begin{itemize}
            ...
            \begin{compactenum}
                ...
                \item if there is any inner descendant position \(\rho'\) of \(\rho\) with \(\varphi(\rho) = \varphi(\rho')\), then each position \(\omega \in \descendants_\xi(\rho) \cap \ancestors(\rho')\) between \(\rho\) and \(\rho'\) has the same value \(\varphi(\rho) = \varphi(\omega) = \varphi(\rho')\).\todo{is there an easier restriction combining ii and iii?}
            \end{compactenum}%
...
%        \(\guide(\xi)\) denotes the set of all guides for the indexed tree \(\xi\).
    ...
    \begin{minipage}[t]{.7\linewidth}
        ...
        \begin{inparaenum}
            \item for each subtree, there is at most the leaf assigned to the root that is assigned to another ancestor; and
    ...
    A guide produced by this constructor maps each node position either to the rightmost\footnote{
        In the publication, this constructor mapped to the leftmost leaf that is a direct successor.
...
%        \mathrm{van}(\xi)(\rho \cdot 1) & \text{if $L_\rho = \emptyset$ and $|\children(\rho) \cap \pos(\xi)| = 1$} \\
    ...
    \begin{proof}
        \begin{enumerate}
            ...
            \item
                The second and third cases in the definition are of relevance for the unitary property.
                The second case is only effective if \(L_\rho \neq \emptyset\).
                Then we assign a leaf that was not yet assigned to an ancestor; it can also not be assigned to any non-leaf descendant, because it is a direct child.
    ...
    As the previous, this constructor is defined for binary constituent structures.
...
%        \mathrm{strict}(\xi)(\rho) = \min \; \yield(\xi|_{\rho\cdot 2})
    ...
    \end{inparaenum}
    Since both of these guide constructors define surjective functions, the number of inner nodes must be less or equal to the number of leaves in each subtree of the constituent structure.
    ...
    This also requires special treatment with respect to binarization which is applied to the constituent structures for the two following guide constructors.
    ...
    Therefore, we limit this guide constructor to the cases of plain (i.e.\@ without rank transformation) or head-inward transformed (cf.\@ \cref{sec:extraction:bin:hi}) constituent trees.\todo{Discuss head-outward binarization shortly}
    ...
    \begin{proof}
        ...
        If \(\head(\xi|_\rho) = \head(\xi|_{\rho'})\), we see again easily the aforementioned property that this leaf must also be the head at all positions between \(\rho\) and \(\rho'\).
    \end{proof}
    ...
    \begin{definition}[Nonterminal Constructor]
        Consider an alphabet \(\varGamma\) (of constituent symbols), a nonempty set \(N\) (of nonterminal symbols), and a \(\DN_+\)-indexed partition \(\mathit{fo}\) of \(N\) (that partitions the nonterminals by fanout).
    ...
    The partition \(\mathit{fo}\) of the nonterminal symbols and its connection to the definition of each nonterminal constructors assure that the constructed nonterminals are considered with consistent fanouts.
    ...
    It combines the super- and subscript of the vanilla nonterminals and annotates constituent symbols with their sum.
    Moreover, the each occurrence of a constituent symbol that is the product of a merge during a rank transformation (cf.\@ merging a chain of unary nodes in \cref{sec:extraction:binarization}) is replaced by the topmost constituent symbol of such chain.
    ...
    \end{inparaenum}
    The latter case roughly approximates the first one when such a table is not available.
...
%    For our examples, the nonterminals are \(\text{S}\) and \(\text{V}\).
    ...
    \begin{table}
        \caption{\label{tab:coarse-nonterminals}
            Clusters of constituent symbols as used in a coarse-to-fine parsing approach. \citep{Cha06} The authors extract coarse grammar rules using the cluster names instead of constituent symbols as nonterminals.
    ...
    \subsection{Lexical Derivation Extraction}
    This small subsection describes how we define a derivation of lexical \abrv{hg} rules using the discussed prerequisites.
    Let's assume we have some constituent structure \(\xi\), a guide \(\varphi\) for \(\xi\) and a nonterminal constructor \(\psi\).
    As the extracted rules are lexical, there is exactly one for each leaf in \(\xi\) and we define a sequence of rules \((r_i \in ??? \mid i \in \yield(\xi))\) that occur in the derivation such that \(r_i\) is a lexical rule containing the symbol \(i\).
    Let us define an extension for the guide \(\phi\) that assigns to each position \(\rho\) the greatest set of leaves \(L\) such that the topmost position that each \(i \in L\) is assigned to is at or below \(\rho\):  \(\hat{\phi}(\rho) = \yield(\xi|_\rho) \setminus \{ \phi(\rho') \mid \rho' \in \ancestors(\rho) \}\).
    Now let \(\rho\) be the topmost position that is assigned \(i\): \(\rho = \min^\unlhd \{\rho' \in \pos(\xi) \mid \phi(\rho') = i\}\) and \(s\in\DN\) and \(\rho_1, \ldots, \rho_s\) the sequence of topmost descendant positions assigned to another leaf and ordered by least assigned leaf, i.e.\@
    \begin{compactitem}
        \item for each \(j \in [s]\) and parent \(\hat{\rho_j}\) of \(\rho_j\), \(\phi(\hat{\rho_j}) = i\) and \(\phi(\rho_j) \neq i\), and there is no other \(\rho_{s+1}\) which fulfills these two properties, and
        \item for each \(j,k \in [s]\) with \(j<k\) holds \(\min \hat{\phi}(\rho_j) < \min \hat{\phi}(\rho_k)\).
    \end{compactitem}
    Then the rule \(r_i\) is of the form \(A_0 \to c_1 \, c_2\; (A_1, \ldots, A_s)\) with
    \begin{compactitem}
        \item \(A_i = \psi(\xi, \rho_i, \hat{\phi}(\rho_i))\) for each \(i \in [s] \cup \{0\}\),
        \item \(c_1 = \mathrm{comp}(\hat{\phi}(\rho_0), \hat{\phi}(\rho_1), \ldots, \hat{\phi}(\rho_s))\), and
        \item \(c_2' = \mathrm{scomp}(\xi, \rho_0, \rho_1, \ldots, \rho_s)\).
            If the position of the leaf \(i\) is a descendant of a \(\rho_i\), then \(c_2\) is obtained from \(c_2'\)  by replacing \(x_i\) by \(x_i(i)\), otherwise \(c_2 = c_2'\).
    \end{compactitem}
    The form of the derivation is determined by \(\xi\) and the guide \(\varphi\), such that
    \begin{compactitem}
        \item the root is the rule containing the position \(\varphi(\varepsilon)\),
        \item for each inner node position \(\rho \in \npos(\xi)\) and child \(\rho'\) of \(\rho\) that is either an inner node with \(\varphi(\rho) \neq \varphi(\rho')\) (or a leaf with \(\rho' \notin \im(\varphi)\)), the rule extracted for \(\phi(\rho')\) (or \(\xi(\rho')\) respectively) is a child of the rule extracted for \(\phi(\rho)\).
    \end{compactitem}
    It is not needed for the extraction procedure anyways, we just care for the sequence of lexical rules.
