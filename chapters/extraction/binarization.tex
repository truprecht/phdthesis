...
\begin{document}
    ...
    \todo{add an example of successor distributions in a treebank, together with the effects on extracted grammar sizes and find citations for generalization capabilities}
    Most grammar extraction procedures construct a rule for each inner node (or a set of inner nodes) in a constituent tree with the rank matching the number of successors of the node (or the sum of successor in the set of nodes).
    ...
    The symbols at these artificial nodes are determined by a chosen \deflab<binarization>[bin:strat]{binarization strategy}, that determines the structure of atr and Markovization parameters. \citep{Son08,Cra12}
    ...
    It reverses the concept of the head-outward binarization strategy to incorporate the generalization effects of binarzation into dependency grammars.
    ...
    As shown in \cref{sec:extraction:dep,sec:experiments:dep}, we experimented with grammar extraction strategies that are common in the field of dependency parsing.
    These grammars tend to be rather large and the afore mentioned binarization strategies either fail to decrease their size and/or there is no intuitive strategy to assign head positions to the introduced artificial nodes, this issue is described in more detail in \ref{sec:extraction:dep}.
    ...
    It can be seen as the opposite to head-outward binarization, because when a node is expanded with artificial children, it does keep the child carrying the head at the top-most position.
